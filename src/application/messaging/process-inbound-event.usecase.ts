...
export interface ProcessInboundEventInput {
  provider: string;
  externalId: string;
  sender: string;
  recipient: string;
...
export async function processInboundEvent(prisma: PrismaClient, input: ProcessInboundEventInput) {
  z.object({
    ...
    recipient: z.string().trim().min(1).max(32),
  }).parse(input);
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      const message = await tx.inboundMessage.create({
        data: {
          ...
          ...(input.providerMetadata !== undefined ? { providerMetadata: input.providerMetadata as Prisma.InputJsonValue } : {}),
          ...(input.correlationId !== undefined ? { correlationId: input.correlationId } : {}),
        },
      });
      await recordAuditLog(tx, {
        actor: input.actor ?? "system:inbound",
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      const message = await prisma.inboundMessage.findUniqueOrThrow({
