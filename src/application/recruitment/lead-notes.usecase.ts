...
export async function deleteLeadNote(prisma: PrismaClient, input: { noteId: string; actor: string }): Promise<Result<LeadNote, DomainError>> { return prisma.$transaction(async (tx) => { const note = await tx.leadNote.update({ where: { id: input.noteId }, data: { deletedAt: new Date() } }); await recordAuditLog(tx, { actor: input.actor, action: "LEAD_NOTE_DELETED", entityType: "LeadNote", entityId: note.id }); return ok(note); }); }
