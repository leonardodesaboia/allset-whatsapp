...
export async function searchLeads(prisma: PrismaClient, term: string): Promise<RecruitmentLead[]> { const q = term.trim(); if (!q) return []; return prisma.recruitmentLead.findMany({ where: { OR: [{ fullName: { contains: q, mode: "insensitive" } }, { phoneE164: { contains: q, mode: "insensitive" } }, { neighborhood: { contains: q, mode: "insensitive" } }, { notes: { some: { content: { contains: q, mode: "insensitive" }, deletedAt: null } } }] }, take: 50, orderBy: { updatedAt: "desc" } }); }
